...
/**
 * Noah Blake
 * 个人理解：每一个浏览器都有一个自己的ChatEndPoint
 * 而onlineUsers是static的，所以每次调用ChatPoint,onlineUsers中储存的值不会变
 */
...
public class ChatEndpoint {
    ...
    private void broadcastMsgToAllOnlineUsers(String message) {
        //所有登录用户名称
        Set<String> names = onlineUsers.keySet();//onlineUsers是Map类型,onlineUsers.keySet()获取所有键,用Set集合储存
        for (String name : names) {
            ChatEndpoint chatEndpoint = onlineUsers.get(name);//获取name对应的ChatEndpoint类
            //获取推送对象
